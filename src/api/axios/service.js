...
service.interceptors.request.use(
  (config) => {
    ...
    if (!notAddToken(url) && notEmpty(store.getters.token))
...
service.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (status === 401) {
      await logout();
      await store
        .dispatch("setExpireLogin", true)
        .then(() => store.getters.expireLogin)
        .catch((res) => res);
 ...
 * */
const notAddToken = (url) => ["login"].some((item) => url === item);
